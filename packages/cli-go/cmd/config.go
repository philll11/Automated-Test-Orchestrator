...
var configSetCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		// SafeWriteConfigAs will create the file if it doesn't exist,
		// and update it if it does.
		if err := viper.SafeWriteConfigAs(configPath); err != nil {
			// If the file doesn't exist, we might get an error.
			// In that case, we try WriteConfigAs, which will create it.
			if os.IsNotExist(err) {
				if err = viper.WriteConfigAs(configPath); err != nil {
					color.Red("Error: Unable to write new config file. %v", err)
					os.Exit(1)
				}
			} else {
